      ...
      <DropdownMenuTrigger
        render={
          <Button
            size="icon"
            className={cn(
              "rounded-sm p-2 flex items-center justify-between size-full",
              !full && "w-auto",
            )}
            variant="ghost"
          >
            <Avatar>
              <AvatarImage
                src={data?.claims.user_metadata?.avatar_url || null}
              />
              <AvatarFallback>
                {data?.claims.user_metadata!.full_name.split(" ")[0][0]}
                {data?.claims.user_metadata!.full_name.split(" ")[1][0]}
              </AvatarFallback>
            </Avatar>
            {full && (
              <span className="flex flex-col items-start truncate">
                <span className="font-bold text-sm leading-tight max-w-35 truncate">
                  {data?.claims.email}
                </span>
