...
export const ProductivityDock: React.FC<ProductivityDockProps> = ({
  ...
  const docTasks = tasks.filter(
    (t) => !t.documentId || t.documentId === document.id,
  );
              ...
              {[1, 2, 3, 4].map((i) => (
                <span
                  ...
                  className={`w-1.5 h-1.5 rounded-full ${
                    i <= completedSprints ? "bg-primary" : "bg-muted/20"
                  }`}
                  title={`Sprint ${i}`}
                />
              ))}
          ...
          <div className="flex items-center justify-between">
            <div>
              <div
                className={`text-3xl font-mono font-black tracking-tight ${
                  isUrgentTimer ? "text-primary" : "text-foreground"
                }`}
              >
              ...
              <p className="text-[10px] font-mono text-[#f2e0d2]/50 mt-0.5">
                {isUrgentTimer
                  ...
                  : `${completedSprints} of 4 Sprints Today`}
              </p>
            </div>
            ...
            <div className="flex items-center space-x-2">
              <Button
                ...
                className={`px-3.5 py-2 rounded-lg font-mono font-bold text-xs uppercase tracking-wider flex items-center space-x-1.5 transition-all shadow-md ${
                  isTimerRunning
          ...
          <div className="space-y-4">
            ...
            >
              ...
              <Button
                ...
                onClick={onToggleDictation}
                className={`w-full py-2.5 rounded-lg text-xs font-mono font-bold uppercase tracking-wider flex items-center justify-center space-x-2 transition-all shadow-md ${
                  isDictating
            ...
            <div className="p-3.5 rounded-xl bg-background/5 border border-border space-y-3">
              ...
              <div className="flex items-center space-x-3">
                <Button
                  onClick={onToggleTTS}
                  className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 transition-transform hover:scale-105 shadow-md ${
                    isPlayingTTS
                      ? "bg-primary text-background"
                      : "bg-foreground hover:bg-foreground/90 text-background"
                  }`}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between text-[11px] font-mono text-[#f2e0d2]/70 mb-1">
                    <span>
                      {isPlayingTTS ? "Reading Draft..." : "Ready to listen"}
                    </span>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] font-mono text-primary">
                {Math.round(
                  (completedTaskCount / (docTasks.length || 1)) * 100,
                )}
                % Complete
              </span>
            </div>
                ...
                >
                  <div className="flex items-start space-x-2.5 flex-1 min-w-0 pr-2">
                    ...
                    <div className="min-w-0">
                      <p
                        className={`text-xs font-sans leading-snug wrap-break-word ${
                          task.completed
